...
echo $i"|"$me"2."$i"|"$pur"SPAM WA BUKALAPAK        "$i"|"$ku"P"$i"|"$pur"          T H A N K S  T O  CRACK           "$i"|"
echo $i"|"$me"3."$i"|"$pur"SPAM CALL LIMIT 3        "$i"|"$ku"A"$i"|"$pur"          Team : DEAFCER TO EXPLOIT               "$i"|"
echo $i"|"$me"4."$i"|"$pur"SPAM CALL LIMIT 5        "$i"|"$ku"M"$i"|"$pu"          YOUTUBE : NO YTN System1              "$i"|"
